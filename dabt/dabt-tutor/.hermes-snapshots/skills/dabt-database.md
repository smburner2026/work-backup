...
**⚠️ Data quality note:** The 2000Q Bank import has known answer-key errors. See `references/data-quality-audit.md` for documented discrepancies by category. When writing explanations or drilling, cross-check the DB's correct_answer_letter against Casarett & Doull reference texts for any question from the 2000Q Bank source before treating the answer as authoritative.
...
See `references/batch-explanation-workflow.md` for the repeatable pipeline used to produce explanations for question batches (Task 3 of the project roadmap). Covers: fetching question data from the DB, crafting 2-4 sentence explanations with mechanism, distractor traps, and source citations, saving structured JSON, and self-review verification.

...
### The Recycle-Bin Pattern (default — use when user says "clean up / organize")
...
### Direct-Deletion Mode (use when user says "delete / nuke / purge")

When the user **explicitly** says to delete, nuke, or get rid of things (not "organize" or "clean up"), skip the recycle bin and delete directly. This mode applies to:

- **AI agent config dirs** — nuke the full canonical list. See `hermes-maintenance` skill for the current list.
- **Old tarballs** — transfer archives like `hermes-transfer.tgz`, `hermes-transfer-bambam.tar.gz` are session artifacts, delete.
- **Session-generated PDFs** — delete all except user-designated keepers (e.g. DABT PDFs).
- **Project-specific stale files** — only after user confirms each category.

Rules when deleting directly:
- Always confirm the deletion scope with the user before executing
- Always verify the deletion completed and report what was removed
- **Never delete without explicit approval** — the distinction between "clean up" and "delete" must come from the user, not your inference
- After bulk moves that precede deletions, always do a residual check (`ls /root/*.py /root/*.json 2>/dev/null`) to catch stragglers
- After nuking agent dirs, always re-scan `ls -la /root/ | grep "^d" | grep "^\."` for new ones that appeared mid-session

See `references/root-directory-audit-and-cleanup.md` for the complete /root/ audit-and-cleanup workflow used this session.
...
- **Loose scripts and reports in `/root/`** — DABT analysis scripts, JSON reports, stray `.py`/`.json`/`.md`/`.sh` files that should be moved into `work/` subdirectories
- **AI agent config directories** — dot-dirs under `/root/` from tools like Cline, Continue, Codeium, etc. See `hermes-maintenance` skill for the canonical list.
- **Old transfer tarballs** — `hermes-transfer*.tgz`, `hermes-transfer*.tar.gz` — session migration artifacts, safe to delete

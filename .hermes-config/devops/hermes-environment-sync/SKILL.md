...
- **VPS→WSL push direction: HMS doesn't work, use raw rsync.** HMS is designed to run FROM the local machine (WSL) — `hms push` goes Local→VPS, `hms pull` goes VPS→Local. When you're ON the VPS and need to push updates TO WSL (e.g., after a gbrain update or skill changes), `hms push` fails because it tries to SSH into the VPS (itself). **Use rsync directly** with the SSH key: `rsync -avz --no-o --no-g --update --exclude='*.hms-bak*' -e "ssh -i ~/.ssh/id_ed25519_vps2local" ~/.hermes/skills/ vthen@100.110.237.89:~/.hermes/skills/`. The raw IP won't match any SSH config host alias, so the explicit key is required. After rsync, verify with `ssh local-machine "find ~/.hermes/skills/ -name '*.md' | wc -l"` to confirm file counts match.

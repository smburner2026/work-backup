...
```bash
# Secret from inline value (MANDATORY — only approach that works)
# Do NOT try @file or - (stdin) syntax — both store the literal string, not the value
BWS_ACCESS_TOKEN="<token>" bws secret create SECRET_NAME "actual-secret-value" <project_id> --note "optional note"
```

**⚠️ CRITICAL: `@file` AND `-` (stdin) syntax DO NOT work.** Unlike many CLI tools, `bws secret create` stores the literal string `@/tmp/secret_val` or `-` as the value rather than reading from file or stdin. **Always pass the value as an inline command argument.**

If the secret value contains special shell characters, use single quotes around it (not double quotes, not piping):

```bash
# Correct — inline with single quotes for shell-safe passing
BWS_ACCESS_TOKEN="<token>" bws secret create SECRET_NAME 'value-with-$pecial-chars!' <project_id> --note "optional note"
```

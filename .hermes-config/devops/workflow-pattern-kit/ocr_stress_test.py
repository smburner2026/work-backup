...
Run:  python3 ocr_stress_test.py
"""
...
# ── Seed data: 3 synthetic OCR pages ──────────────────────────────
...
class OCREngine:
    """A simulation that deliberately introduces realistic OCR defects."""
    ...
    async def extract(self, image_path: str, file_system: FileSystem) -> str:
        ...
        if not content:
            return ""
        # Simulate 0.5s processing per page
        await asyncio.sleep(0.5)
        # Simulate common OCR errors: swapped characters
...
async def ocr_page(params: OCRParams, file_system: FileSystem = None):
    """Extract text from a source page using the OCR engine."""
    engine = OCREngine()
...
async def clean_text(text: str):
    """Fix simulated OCR errors: character swaps, typos."""
...
async def translate(text: str, target_lang: str = "en"):
    """Simulate translation (just prepend English marker)."""
    return f"[EN]{text}[/EN]"
...
async def write_output(content: str, output_path: str, file_system: FileSystem = None):
    """Write the final cleaned+translated output."""
    path = await file_system.write(output_path, content)
    return path


# ── LoopDetector for action tracking ──────────────────────────────
...
async def run_stress_test():
    ...
    async def ocr_1(file_system):
        ...
        return await registry.execute("clean_text", {"text": raw}, {"file_system": file_system})
    ...
    assert "Ngô Quyền" in final, "OCR missing expected content"
    assert "dựng nước" in final, "OCR cleanup didn't fix characters"

    print(f"\n  ✅ ALL 3 PAGES OCR'd AND SYNTHESIZED IN PARALLEL")
    print(f"  ⏱  Wall time: {t_dag:.2f}s (3 pages × 0.8s each would be ~2.4s sequential)")
    print(f"  📊 Pipeline status: {result.task_statuses}")
    print(f"  📝 Output size: {len(final)} chars")
    ...
    bad_outputs = [
        "",                                                                                  # empty
        "Phase 1: OCR the pages. Phase 2: Clean the text. Phase 3: Translate.",              # plan stub
        '{"status": "ok", "content": "done"}',                                              # raw envelope
        "I'll help you with that OCR task by extracting text from the provided pages.",      # vague
    ]
    ...
    for i, bad in enumerate(bad_outputs):
        ...
        if reason:
            print(f"  🔴 Caught [{i+1}]: \"{bad[:50]}...\" → {reason}")
            caught += 1
        else:
            print(f"  🟢 Passed [{i+1}]: \"{bad[:50]}...\"")
    ...
    assert statuses["good_1"] == "completed", "good_1 should complete"
    assert statuses["fails"] == "failed", "fails should fail"
    assert statuses["downstream"] == "skipped", "downstream should be skipped"
    print(f"  ✅ Error isolation works: failed task didn't crash good task")

    # ── Phase 4: LoopDetector during retries ──────────────────────
    print("\n\n📋 PHASE 4: LoopDetector — Stuck Action Detection")
    print("-" * 48)

    dl = LoopDetector(window_size=10)
    for i in range(12):
        dl.record_action("click", {"index": 3})  # same action repeatedly

    print(f"  📊 Window: {len(dl.recent_action_hashes)} actions, "
          f"max repeat: {dl.max_repetition_count}x")

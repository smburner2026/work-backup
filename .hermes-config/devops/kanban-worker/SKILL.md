...
**MCP servers spawn per-process.** Each Hermes subprocess (including kanban workers) starts its own copies of every MCP server defined in `mcp_servers` config. If the gateway already has tradingview-mcp or wundertrading running, a spawned worker creates duplicates — wasting ~100MB per duplicate pair. Mitigation: either run such MCP servers as standalone daemons outside Hermes's lifecycle, or remove them from the worker profile's config scope so they aren't auto-started on spawn.
...
> **CLI workflow recipes** — the mapping above covers individual commands, but the create→assign→dispatch→inspect **workflow** has its own gotchas (priority-as-int, unassigned cards being silently skipped, dispatch not taking a task-id). See `references/kanban-cli-recipes.md` for the full recipe, pitfalls table, and flag reference.

...
Every cron job should run with the **minimum toolset** it needs. This reduces token overhead (30-50% savings on input), keeps the agent focused on its actual task, and prevents accidental misuse of tools irrelevant to the job.
...
2. **Map each action to the corresponding Hermes toolset:**
   ...
   - Load domain skills → `skills`
   - Python analysis → `code_execution`
3. **Exclude everything else** — tools the job does not need are not granted
4. **Tighten aggressively, loosen on failure** — user heuristic: "Worst case if he needs more tools, we can turn them on." A job that fails because a tool was missing surfaces immediately and is trivially fixable. A job that silently misbehaves because it had too many tools may never be caught.
...
See `references/cron-tool-scoping-examples.md` for concrete audit records from production runs.

### Pitfalls

- **`web` is over-scoped by default** — most maintenance/audit crons do not need external lookups. Only grant it when the job explicitly cross-references online sources.
- **`messaging` is almost never needed** — the cron scheduler handles delivery automatically. The agent produces content; the framework delivers it.
...
When a task has multiple independent phases (e.g., catalog → download → index → sync), **execute one phase at a time with output between**. This prevents the "overstretch" failure mode where a single turn tries to do everything and hits timeouts.
...
Each phase should:
- Produce visible output the user can see and approve
- Be independently verifiable (check file sizes, counts, exit codes)
- Have a clear "done" state before the next phase begins

### When NOT to use this pattern
- Trivial 1-2 step tasks (just do it directly)
- Emergency fixes where the user explicitly says "just fix it now, no need to show me"

## Hybrid Async Pattern (Cron + Background Terminal)

For complex multi-phase tasks, run complementary async methods simultaneously:
...
- Background terminal handles the fast, local script-based parts (file imports, metadata updates)
- Neither blocks the LLM turn — the conversation continues while both run
- Results from each arrive independently as they finish

### Implementation pattern
```python
# 1. Write the script for local batch work
write_file("/tmp/import.sh", script_content)

# 2. Start background terminal (imports existing files)
terminal(
    command="bash /tmp/import.sh",
    background=True,
    notify_on_complete=True
)

# 3. Create cron job for web-dependent work
cronjob(
    action='create',
    name='Project - Web Downloads',
    schedule='1m',
    repeat=1,
    deliver='origin',
    enabled_toolsets=['web', 'terminal', 'file'],
    prompt='''Download X documents...'''
)

# 4. Continue conversation — both deliver results when done
```

### Troubleshooting
- **Background terminal produces no output** → bash may be buffering. Add `stdbuf -oL` or `unbuffer` to the command, or use `| tee /dev/stderr` to force line-buffered output.
- **Cron job doesn't fire** → check schedule format. `'1m'` works, `'0m'` does not. Check for paused or conflicting jobs via `cronjob(action='list')`.
...
See `references/cron-debugging-checklist.md` for a systematic 8-step checklist when a cron job doesn't fire, doesn't deliver, or fires at the wrong time. Covers: verifying the job exists, checking last_run_at, comparing schedules against user timezone, checking the scheduler daemon, delivery target thread_id issues, manual test runs, and reading output logs. Use this when a user reports a cron job missed its window.
...
- **Explicit data-source instructions required** — A cron agent has ZERO session context. If the job needs to produce output from a data file, the prompt MUST explicitly say "Read file X" with the full absolute path. The agent will NOT infer it from the skill alone. Failure to include this is the #1 cause of empty cron output.

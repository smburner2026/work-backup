...
## Using Kanban in Hermes — Quick Start

> This section is specifically about **Hermes Agent's built-in kanban system** — the task board that runs in SQLite on your machine. If you're here because a conversation loaded the kanban-guru and you don't know how to start, this is for you. The rest of this skill covers Kanban *methodology*; this section covers how to *use* it.

### It's Already Running — No Setup Needed

Hermes Kanban is a task tracker with exactly **four states** a card moves through:
...
No columns to configure. No WIP limits to write. No visual board to install. It's always-on SQLite — you can create cards the moment Hermes is running.
...
```
 t_abc1234  todo      default     Research: Genotoxicology mechanisms
 t_def5678  ready     default     Review open BTC positions
 t_ghi9012  doing     default     Morning market snapshot
 t_jkl3456  done      default     Plan today's study session
 t_mno7890  blocked   default     Adjust TP on open short
```

Each row is a card with an ID, a state, an assignee, and a title. The "board" is this list filtered by state. Cards start in `todo`, you pull them through states as work progresses.

### Profiles and How They Work

When you assign a card to a **named profile** (like `mike`, `euphy`, or any profile you configure), the kanban dispatcher spawns that profile as a fresh agent to handle the card — with its own model, tools, and working directory. This enables:

- **Parallel work** — 3 cards assigned to 3 profiles run concurrently instead of one-at-a-time
- **Specialization** — a `mike` profile with DABT study tools handles exam prep while the main agent handles trading
- **Isolation** — each profile gets its own terminal session and memory context

But again: **all of this is optional.** Assigning every card to `default` gives you a perfectly functional task tracker.

### Concrete Session Example

Real workflow from this session's DABT study plan:

```\nhermes kanban create "Plan today's study session" --assignee default\nhermes kanban create "Review Carcinogenesis pathways" --assignee default\nhermes kanban create "Create flashcards: DNA Repair" --assignee default\n\n# See the board:\nhermes kanban list\n\n# Start working:\nhermes kanban transition t_xxx doing\n\n# Mark complete:\nhermes kanban complete t_xxx\n\n# Something's stuck:\nhermes kanban block t_xxx "Waiting on source PDF"\nhermes kanban unblock t_xxx        # unstick when ready\n```

### Card Lifecycle Reference

See `references/hermes-kanban-commands.md` for the full command reference with all options, filters, and workflow patterns.
...
|---|---|
| First time using Hermes Kanban | `references/hermes-kanban-commands.md` |

...
## Fallback: Extracting Tweet Content Without xurl

When `xurl` is not installed, unavailable, or the API is rate-limited, use this curl-based fallback to extract tweet text and attached media directly from X.com's server-rendered HTML.
...
The `window.__INITIAL_STATE__` JSON embedded in the page's HTML contains the full tweet data, including:
- `full_text` — tweet body text (html-escaped, e.g. `&amp;gt;` for `>`)
- `entities.media[].media_url_https` — URLs of attached images
- `user.name`, `user.screen_name` — author info
- `favorite_count`, `retweet_count`, `reply_count` — engagement metrics
- `created_at` — ISO timestamp
- `display_text_range` — character range of the text portion (before media)

For a complete extraction (all fields), pipe the full `__INITIAL_STATE__` to Python for structured parsing:

```bash
curl -s -L "https://x.com/$USERNAME/status/$TWEET_ID" | \
  grep -oP '"tweets":\{"entities":\{[^}]+' 
```

### Download attached images

Parse the media URL and download:

```bash
MEDIA_URL=$(curl -s -L -H "User-Agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36" \
  "https://x.com/$USERNAME/status/$TWEET_ID" | \
  grep -oP '"media_url_https":"[^"]*"' | head -1 | cut -d'"' -f4)
curl -s -o /tmp/tweet-media.jpg "$MEDIA_URL"
```

Then use `vision_analyze` on the downloaded file to extract text visible in screenshots, diagrams, or infographics.

### Write access

This fallback is **read-only**. For any write operation (post, reply, like, repost, follow), install and configure `xurl` following the Installation + One-Time User Setup sections above.

### Limitations

- Only works for logged-out X.com pages — private/protected tweets do not render.
- The `__INITIAL_STATE__` structure can change if X updates their frontend rendering — the grep patterns may need adjustment.
- Does not capture reply threads or nested conversations — only the primary tweet.
- Rate-limited by X's CDN (typical limits are higher than the API but still present).

---

## Agent Workflow

1. **Try `xurl read` first** — if `xurl` is installed and authenticated, prefer it. It is faster, more reliable, and returns structured JSON.
2. Fall back to the curl+grep approach above only if `xurl` is unavailable or fails.

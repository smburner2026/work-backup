...
### ⚠️ gbrain is independent of Hermes's provider setup

**gbrain does NOT use Hermes providers or ACP transports (opencode-go, pi, claude-code, codex).** It is a standalone Bun/TypeScript CLI with its own model gateway. gbrain needs its own API key in its own environment — it cannot "reuse" whatever provider Hermes is routing through.

- The API key you set for gbrain must be in gbrain's runtime environment, not just Hermes's config
- See the `gbrain` skill's "Independent from Hermes Provider Setup" section for the full explanation
- If you're unsure what API key to use, the simplest path is `OPENROUTER_API_KEY` (and use the free NVIDIA embedding model)

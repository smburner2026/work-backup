...
5. **Load the relevant skill** — If the user mentioned a tool by name (gbrain, pi, lightpanda, etc.) and you have a skill for it, load it with `skill_view(name)`. The skill may contain setup state, pitfalls, and config that your memory notes don't capture. **Crucially: follow the skill's instructions.** Loading a skill and then using a default tool instead of the one the skill prescribes wastes context and undermines the skill system. The skill's tool choice and workflow supersede your defaults. The dream-cycle Claude question below was avoidable if the gbrain skill had been loaded before proposing directions.

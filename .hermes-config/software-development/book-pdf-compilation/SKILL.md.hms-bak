...
### Sourcing Existing PDFs from Public Archives

When the target is a known-public-domain work and you need a clean PDF (not markdown/OCR), check these archives:

- **Online Library of Liberty (oll.libertyfund.org)** — born-digital text PDFs, not scans. The site is JS-heavy (Turbo), so probe the S3 bucket directly: `https://oll-resources.s3.us-east-2.amazonaws.com/oll3/store/titles/{title_id}/{slug}_EBk_v6.0.pdf`. Title IDs are sequential per set (e.g. the 5-volume Dryden Plutarch uses 1771–1775). Find the set page title on OLL, then brute-force the S3 URL pattern.
- **Project Gutenberg (gutenberg.org)** — clean HTML/text, no direct PDF. Use the HTML version and save-as-PDF from browser, or use EPUB/Kindle format.
- **Bill Thayer's LacusCurtius (penelope.uchicago.edu)** — minutely proofread, Loeb translations. Web-based HTML, not PDF.

**Pattern:** When web_search/web_extract fail (e.g. Tavily 432 errors on JS-heavy sites), fall back to `terminal` with `curl -sI` to probe URLs and `curl -sL -o` to download. The terminal backend is a reliable bypass for API rate-limits and JS-rendered pages.

### Merging Sourced PDFs into a Single Volume

After downloading multiple PDF volumes from an archive, merge them with `pdfunite` (part of poppler-utils):

```bash
pdfunite vol1.pdf vol2.pdf vol3.pdf vol4.pdf vol5.pdf complete.pdf
```

This produces a correctly ordered, single-file PDF with preserved text and no re-encoding. The merged file is typically smaller than the original sum due to shared resource deduplication. Verify with PyMuPDF: `doc = fitz.open(path); doc.page_count`.

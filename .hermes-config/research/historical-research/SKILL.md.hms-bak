...
description: "Primary-source historical research with Hermes — document acquisition, OCR/extraction, translation, provenance tracking, and structured data extraction from archival sources."
version: 1.0.0
author: Hermes Agent
metadata:
  hermes:
...
└── reference/         # Downloaded reference documents
```
...
9. **Storage strategy** — After scaffolding, address storage and organization. Save extracted content to the project directory as markdown files. Obsidian for detailed source notes (optional), cloud storage for raw files (PDFs, images).
...
| **Scanned Vietnamese/French (pre-1975)** | **`tesseract` (vie+fra), 300 DPI, --psm 1** | **Multi-lingual OCR for scanned historical texts with mixed Vietnamese and French content. Lighter than marker-pdf (no 3GB model). Setup: `apt-get install tesseract-ocr tesseract-ocr-vie tesseract-ocr-fra poppler-utils`. Batch: `pdftoppm -r 300 input.pdf /tmp/pages && for f in /tmp/pages-*.ppm; do tesseract $f stdout -l vie+fra --psm 1 >> output.txt; done`. Concatenate with `=== PAGE N ===` markers for page tracking.** |
| Vietnamese-only (modern) | `tesseract` (vie) | Single-language OCR for 20th-century Vietnamese texts |
...
- **Sub-agent timeout for long-running OCR:** `delegate_task` has a 600-second (10-minute) soft timeout. OCR of a 500-page volume at 300 DPI takes 15-25 minutes. Do NOT use `delegate_task` for per-volume OCR — launch background terminal processes instead: `terminal(command, background=true, notify_on_complete=true)`. Each volume in its own process. Monitor with `process(action='poll')`.
- **Background OCR heat:** Running 7 parallel tesseract instances (~300-500MB RAM each) on a 38GB VPS is fine but will saturate CPU. Use `nice -n 19` to keep the system responsive. Write a parameterized shell script per volume rather than inline commands for reproducibility.

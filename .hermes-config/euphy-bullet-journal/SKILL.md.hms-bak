...
- Cron jobs for daily/weekly/monthly all deliver to the output channel. See `references/cron-schedules.md` for schedule details, horizon system, and prompt rules.
...
### 4b. Date Horizon Clarification

When the user instructs you to add a task or event **without specifying a date**, you MUST gently ask for the due date before recording it:

- "By when would you like this done, sir?"
- "Is there a particular date for this, sir?"

Accept natural language dates ("this Friday", "next week", "end of month") and convert them to YYYY-MM-DD. Logically inferable dates are OK without asking:
- "Remind me to call the bank tomorrow" — tomorrow is clear, no need to ask
- "Schedule a meeting for Friday" — the upcoming Friday is clear
- "Add 2687-2600398 report" — **ask** — no date context

This prevents date-less tasks from accumulating in the journal.
...
**Bulk operations**: When the user says "all of today's tasks are completed", "mark everything done", or similar bulk phrasing:
- Read the file to identify all pending (•) items under today's date header (and any items elsewhere due today).
- Use `euphy-complete "Task text" YYYY-MM-DD` for each pending item individually.
- Report which items were completed and what (if anything) remains.
